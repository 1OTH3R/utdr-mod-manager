if (!fs.existsSync(modJsonPath)) {
    const defaultData = {
        "mods": [
        {
            "name": "This is the Mod Manager!",
            "creator": "1OTH3R",
            "icon": "data:image/png;base64,",
            "desc": "This is a mod!",
            "path": undefined,
            "img": "data:image/png;base64,",
            "info": "Yolo!!",
            "exe": undefined
        }
        ]
    };
    fs.writeFileSync(modJsonPath, JSON.stringify(defaultData, null, 2), 'utf8');
}

function scan_exe(the_mod) {
    const ze_path = the_mod.path;
    
    if (!ze_path) return 1;

    let files = fs.readdirSync(ze_path, { recursive: true });
    let exeFiles = files.filter(filename => path.extname(filename) === '.exe');

    if (exeFiles.length > 0) {
        let formattedExeList = exeFiles.map(file => [
            path.basename(file),
            path.join(ze_path, file)
        ]);
        
        return formattedExeList;
    } else {
        console.log(`Ruh roh! I couldn't find any exe in here, but I did find a path! (Path: ${ze_path})`);
        return 1;
    }
}

const data = JSON.parse(fs.readFileSync(modJsonPath, 'utf8'));
them_mods = ``;
data.mods.forEach(mod => {
    them_mods += ` '${mod.name}'`;
    scan_exe(mod);
    showMod(mod.name, mod.creator, mod.icon, mod.desc, mod.path, mod.img, mod.info, mod.exe, parseInt(mod.path?.match(/\d+$/)?.[0]), false); // This returns the ID/NaN
});
console.log("Mod(s) detected:", them_mods);
